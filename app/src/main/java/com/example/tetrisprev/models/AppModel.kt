package com.example.tetrisprev.models

import android.graphics.Point
import com.example.tetrisprev.constants.FieldConstants
import com.example.tetrisprev.constants.CellConstants
import com.example.tetrisprev.helpers.array2dOfByte
import com.example.tetrisprev.storage.AppPreferences

class AppModel {
    enum class Statuses {
        AWAIRTING_START, ACTIVE, INACTIVE, OVER
    }
    ...
    var currentState: String = Statuses.AWAIRTING_START.name
    ...
    fun isGameAwaitingStart(): Boolean {
        return currentState == Statuses.AWAIRTING_START.name
    }
        ...
        return if (position.y < 0 || position.x < 0) {
            ...
            for (i in 0 until shape.size) {
                for (j in 0 until shape[0].size) {
                    ...
                    val x = position.x + j
                    if (
                        CellConstants.EMPTY.value != shape[i][j] &&
    ...
    private fun moveValid(position: Point, frameNumer: Int?): Boolean {
        val shape: Array<ByteArray>? = currentBlock?.getShape(frameNumer as Int)
    ...
    private fun assessField() {
        for (i in 0 until field.size) {
            var emptyCells = 0
            for (j in 0 until field[i].size) {
                if (CellConstants.EMPTY.value == getCellStatus(i, j)) {
                    emptyCells++
                }
            }
            if (emptyCells == 0) {
    ...
    private fun shiftRows(nToRow: Int) {
        ...
        for (m in 0 until field[0].size) {
            setCellStatus(0, m, CellConstants.EMPTY.value)
        }
    }

    private fun translateBlock(position: Point, frameNumer: Int) {
        synchronized(field) {
            val shape: Array<ByteArray>? = currentBlock?.getShape(frameNumer)
            if (shape != null) {
    ...
    fun resetModel() {
        resetField(false)
        currentState = Statuses.AWAIRTING_START.name
        score = 0
    }
}