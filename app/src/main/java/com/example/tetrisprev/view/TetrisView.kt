package com.example.tetrisprev.view
...
class TetrisView : View {
            ...
            if (msg.what == 0) {
                if (owner.model != null) {
                    if (owner.model!!.isGameOver()) {
                        owner.model?.endGame()
                        Toast
                            .makeText(owner.activity, "Игра окончена", Toast.LENGTH_LONG)
                            .show()
                    }
                }
    ...
    private fun updateScores() {
        activity?.tvCurrentScore?.text = model?.score?.toString()
        activity?.tvHighScore?.text = activity?.appPreferences?.getHighScore()?.toString()
    }
